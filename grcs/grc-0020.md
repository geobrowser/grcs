...
- **Total ordering** - On-chain publishing provides provenance and global consensus over state
...
**Publishing.** The edit is serialized to a compact binary format (see `grc-0020-encoding.md`), published to content-addressed storage (IPFS), and its hash is recorded on-chain. Indexers replay accepted edits in governance-defined order, building the resolved state of the space deterministically.
